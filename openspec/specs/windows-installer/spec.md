...
- **WHEN** the user executes `omnidesk install` or runs the setup installer
- **THEN** the system SHALL register the OmniDesk executable under `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` and verify the registration.

#### Scenario: Disabling autostart on Windows
- **WHEN** the user executes `omnidesk uninstall` or runs the uninstaller
- **THEN** the system SHALL remove the OmniDesk key from `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` and terminate the background process.

### Requirement: Windows Setup Wizard Packaging
The system SHALL provide an NSIS script and build script to compile a standalone graphical setup installer (`OmniDesk-Setup-<version>-x64.exe`).

#### Scenario: Executing the setup wizard
- **WHEN** the user runs the setup installer on Windows
- **THEN** the wizard SHALL guide the user, install `omnidesk.exe` into `%LOCALAPPDATA%\Programs\OmniDesk\`, create Start Menu and Desktop shortcuts, and configure autostart.

#### Scenario: Registering Windows uninstaller
- **WHEN** the setup wizard completes installation
- **THEN** the system SHALL create `uninstall.exe` and register OmniDesk in Windows "Installed Apps" / "Add or Remove Programs" for clean uninstallation.
...
- **WHEN** the user selects "Abrir Painel (Dashboard)" from the Windows system tray or executes `omnidesk gui`
