...
DMG_PATH="${DIST_DIR}/OmniDesk_${VERSION}_${TARGET_ARCH}.dmg"
ZIP_PATH="${DIST_DIR}/OmniDesk_${VERSION}_${TARGET_ARCH}.zip"
...
cp "${ROOT_DIR}/scripts/install-mac.command" "${STAGING_DIR}/Instalar OmniDesk.command"
chmod +x "${STAGING_DIR}/Instalar OmniDesk.command"
...
if command -v hdiutil >/dev/null 2>&1; then
    ...
    rm -f "${DMG_PATH}"
    hdiutil create -volname "OmniDesk" \
...
rm -f "${ZIP_PATH}"
cd "${STAGING_DIR}"
zip -r -y -q "${ZIP_PATH}" "OmniDesk.app" "Applications" "Instalar OmniDesk.command"
cd "${ROOT_DIR}"
rm -rf "${STAGING_DIR}"
