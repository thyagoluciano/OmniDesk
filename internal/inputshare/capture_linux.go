...
func (b *x11Backend) dispatchMotion(raw []byte) {
	...
	if b.suppressed.Load() && b.warpPending.Load() {
		// Ignore the synthetic motion event generated by our own WarpPointer to center.
		if ev.RootX == cx && ev.RootY == cy {
			b.lastX = int32(cx)
			b.lastY = int32(cy)
			b.haveLastXY = true
	...
	if b.suppressed.Load() && !b.warpPending.Load() {
		threshX := int16(b.screenW / 4)
		threshY := int16(b.screenH / 4)
		if ev.RootX < cx-threshX || ev.RootX > cx+threshX ||
			ev.RootY < cy-threshY || ev.RootY > cy+threshY {
